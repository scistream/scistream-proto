...
def parseOptions():
    ...
    parser.add_option('--size', dest='size', type=int, default=1024, help='Sample size in bytes')
    parser.add_option('--dataset', dest='dataset', type=int, default=10, help='Dataset size in Gbytes')
    parser.add_option('--jitter', dest='jitter', action="store_true", default=False, help='Reduce the generation rate to 1 KHz')
...
for i in range(samples):
    ...
    if opts.jitter:
        ...
        if sleep_time > 0:  # If sleep_time is non-negative, then sleep for the remaining duration
