...
def parseOptions():
    ...
    parser.add_option('--pacing', dest='pacing', action="store_false", default=False, help='Reduce the consumption rate')
    parser.add_option('--st', dest='st', type=float, default=0.001, help='Sampling time')
  ...
  def run(self):
      ...
      while True:
          ...
          if message.decode("utf-8")[10] == 'S':
              ...
              logging.info("Elapsed: %s secs. | Throughput: %.3f Gbps | Jitter: %f | Inter-message Space: %f | Efficiency: %.2f%%" % (t, throughput, jitter, avg_ims, efficiency))
              results_log.write("%s,%s,%s,%s\n" % (throughput, jitter, avg_ims, efficiency))
              results_log.flush()
              results_log.close()
